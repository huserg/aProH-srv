                    ...
                    <div class="mt-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                        @foreach (Laravel\Jetstream\Jetstream::$permissions as $permission)
                            <label class="flex items-center">
                                ...
                                <span class="ms-2 text-sm text-yellow-600 dark:text-yellow-400">{{ $permission }}</span>
                            </label>
                        @endforeach
                    </div>
                </div>
                        ...
                        @foreach ($this->user->tokens->sortBy('name') as $token)
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center ms-2">
                                    ...
                                    @if (Laravel\Jetstream\Jetstream::hasPermissions())
                                        <button class="cursor-pointer ms-6 text-sm text-yellow-400 underline" wire:click="manageApiTokenPermissions({{ $token->id }})">
                                            {{ __('Permissions') }}
                                        </button>
                                    @endif
    ...
    <x-dialog-modal wire:model.live="displayingToken">
        ...
        <x-slot name="content">
            ...
            <x-input x-ref="plaintextToken" type="text" readonly :value="$plainTextToken"
                class="mt-4 bg-black-100 px-4 py-2 rounded font-mono text-sm text-yellow-500 w-full break-all"
